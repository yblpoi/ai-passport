...
- 生成结果：
  - `images/love_pixel_art.c` + `main/love_pixel_art.h`：18 个 40×40 的 4bpp
    索引（I4）图标（20×20 逻辑像素放大 2 倍）、48×48 的 I1 爱心底纹平铺砖，
    以及 `love_pixel_palette[16]`。
  - **底纹只画爱心、不画底色**：图里只有"白（索引 1）+ 透明（索引 0）"两色，
    所以能压成 1bpp —— 连调色板一共 **296 字节**，而原来的 RGB565 版本是 4,608 字节。
    底色由**设备 style 的 `bg_color`**（网页端是 `body` 与 `.screen` 的
    `background-color`）在运行时透出来（见 `main/love_app.c` 的 `render()` 与
...
- 代价：图标、底纹与调色板合计约 119 KB 源码 / 18 KB Flash（图标 15.2 KB，
  即 18 张 864 字节的 I4 图；底纹 296 字节）；I4 图标在绘制时按行转换，不整屏缓存。
