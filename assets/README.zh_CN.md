...
- 调色板顺序即自定义头像 4bpp 的**回退**索引顺序（`PALETTE_ORDER`）。每图自带配色
  落地之后，它只作用于：那之前上传的头像、只传索引的上传、以及内置图标 ——
  但**改了仍然会让这些全部换色**，所以是显式写死的而不是依赖 dict 顺序。
...
- 自定义头像的**照片处理在网页端完成**，设备只负责存与打包。内核
  （`assets/web/avatar_pixel.js`）由 node 的 `vm` 直接跑主机测试，并由
  `tools/gen_admin_page.py` 内联进后台页面，所以一次页面加载仍然只有一个 `/admin.js`
  请求。算法、五个可调旋钮、实测精度，以及上传体与存储配色的字节布局，见
  [头像像素化](../docs/development/engineering/avatar-pixelation.zh_CN.md)。
