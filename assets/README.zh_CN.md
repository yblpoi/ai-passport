...
- 图标在 2026-09-18 **整套重画**（16 个）：改成统一的"1px 描边 + 平涂 + 对称轮廓"风格，
  每个在 40px 下都能一眼认出来。改前的 bird 是一团黄色、rabbit 像杯子、dog 的耳朵像犄角、
  tree 的红树干长在树中间、star 读成"胖十字加两条腿"、flower / cake / ring / balloon 也都含糊。
  新掩码由 AI 生图工具出的设计稿作参考、**手工排到 8×8 栅格**（模型的"像素"并不落在真实栅格上、
  描边是发丝细，直接降采样会糊，所以没法自动转）。两轮过程：先改读不出来的 4 个
  （bird/dog/rabbit/tree），确认风格后把其余 12 个也统一过来，其中 **star 画成饱满的五角星**（照 ⭐️ 的形状 —— 上尖 + 左右两臂 + 两条腿，
  早先试过四角"闪光"、也试过收腰的"瘦星星"，用户都不认）。**它是 16 个里唯一不画深色描边的**：
  8×8 里五个尖必须顶到画布边缘才够饱满，收一像素去套描边就会退化成"胖十字加两条腿"；
  外沿换橙色当暗部也试过，看起来像脏掉的渐变。
...
- 头像圆角：半径 **4px**，**只作用于自定义头像**（上传的照片，满幅方角）；
  ...
  **为什么不给内置图标加**：它们是透明背景的**图形**而非方块牌，而四角确实有描边——
  ...
  （给内置图标加同样会切描边），设备端 custom 头像在打包成 I4 时把角落改指到透明索引
