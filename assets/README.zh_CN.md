...
- 头像圆角：半径 **4px**，写在 `ICON_CORNER_RADIUS`。**圆形底座仍然不做**——实测圆形
  会让 16 个角色里 15 个掉实心像素（猫咪少 186 个、礼物少 284 个，耳朵和边角被切平）。
  4px 轻圆角温和得多：四角一共只裁 17 个画布像素，而 16 个图标里 8 个四角本来就是空的
  （它们是透明背景的图形，不是方牌），真正被裁到实心像素的合计只有 64 个——星星/蛋糕/
  礼物各 12、叶子 8、猫/狗/熊/狐狸各 5。这样自定义头像（照片、满幅方角）不会显得生硬。
  半径在三处必须一致：本文件、`main/love_app.c` 的 `AVATAR_CORNER_RADIUS`（自定义头像的
  调色板没有透明项，只能在解码成 ARGB8888 时把角落 alpha 置 0）、`assets/web/admin.css`。
  内置图标走的是"生成时把角落像素改成透明"（调色板索引 0 即透明），所以 I4 数据、导出的
  PNG、网页 data URI 三者自动一致。
