...
- 预览是按设备的 240×320 逻辑像素摆位的（坐标、字号、行高都精确），但字形来自
  浏览器自己的系统字体，**这页没有 `@font-face`**。
- 曾经内嵌过 Ark Pixel 12px 的一个子集（3,891 字形：ASCII 95 + 中日韩标点约 40 +
  GB2312 一级字 3,755，woff2 约 122 KB），让预览与真机是同一套点阵字形。放弃的原因是
  体积：页面自己的固定文案只用到 475 个汉字，为了"任意姓名也能显示像素字形"要背上
  二十倍于页面本体的传输量；改成独立路由 + 一小时缓存后仍嫌重。
- 该文件已从仓库删除（曾经是 `fonts/ark12-subset.woff2`）。真要恢复：源 TTF 见上一节的
  获取方式，然后
  ...
  并同时改 `tools/gen_admin_page.py`（字体输入与字节输出）、`main/love_httpd.c`
  （`/font.woff2` 路由）和 `assets/web/admin.css`（`@font-face`）。注意字符集已不是
  当年的 3,755 字——现在设备字库收的是一级 + 二级共 6,910 字，照 `love_font_12.c`
  的 `Opts:` 行取会得到约两倍大的文件。
...
- **新图标只能往后追加**：2026-09-18 从 16 个扩到 18 个（鞭炮 16、花束 17，日历默认值
  要用），四个自定义头像槽因此从 16..19 挪到 18..21——所以配置版本升到 5：读 v4 及更早
  的记录时，凡是 >= 16 的图标号统一 +2（`main/love_config.c` 的 `icon_v4_to_v5()`，
  有主机测试钉住）。
- 生成结果：
  - `images/love_pixel_art.c` + `main/love_pixel_art.h`：18 个 40×40 的 4bpp
    索引（I4）图标（20×20 逻辑像素放大 2 倍）、48×48 的 A8 爱心底纹平铺砖，
    以及 `love_pixel_palette[16]`。
  - **底纹只画爱心、不画底色**：图里只有"爱心的不透明度"这一层信息，所以存成
    **A8**（每像素 1 字节的遮罩，**2,304 字节**），颜色由 `main/love_app.c` 里 style 的
    `bg_image_recolor`（白）+ `recolor_opa`（满）给 —— 画出来就是"白心上叠着图里那
    30% 的 alpha"。原来的 RGB565 版本是 4,608 字节。
    **为什么不是更小的 I1**：试过，一屏刷新从 104 ms 掉到 **396 ms**（每带 45~56 ms）。
    索引图在没开 `LV_BIN_DECODER_RAM_LOAD` 时是**逐行**解成 ARGB8888 再混合，平铺时
    每带要把上百行各解一遍；A8 走的是 `lv_draw_sw_img.c` 里"把整张图当遮罩、用 recolor
    颜色填充"的快路径，每像素只读 1 字节。实测三种格式的整屏耗时：RGB565 104 ms /
    A8 103 ms / I1 396 ms（同一块板、240×40 缓冲、8 带）。
    底色仍由**设备 style 的 `bg_color`**（网页端是 `body` 与 `.screen` 的
    `background-color`）提供（见 `main/love_app.c` 的 `render()` 与
    `assets/web/admin.css` 的 `--pink`），于是"换底色"只是改一个颜色值，不必重新
    生成素材，也不必重新烧录底纹。头像是同一套思路的另一个例子：图里只留形状与
    颜色，底色/圆角这类可变的部分留给运行时。
  - **每张图标自带**一张 16 色调色板：18 张图用色的并集远超 16 个槽位，而
...
- 头像圆角：半径 **4px**，**只作用于自定义头像**（上传的照片，满幅方角）；
  ...
  **图形**而非方块照片；实测 18 张生成图里只有猫咪的两个上角触到画布边缘，4px 圆角
  只会切到它、其余 17 张毫无变化——这条规则就只会作用于唯一需要注意边角的那个图标。
...
- **细节多少由 step 决定，不是由 mode 决定**：步长 S 相对输出格子（6px）越大，就有越多格子
  共用同一片区域、共用同一个颜色 —— soft(8) 约 1.3 格/区域、normal(14) 约 2.3、
  strong(22) 约 3.6。40x40 一共只有 1600 格，normal 的实际空间分辨率只剩约 17x17。
  默认因此给 **soft**。这一点是用户反馈"完全没有细节"之后才理清的，一度被归错到"迭代次数"上。
- `mode` 决定**一片区域该填什么色**（四个取值）：
  **cell** 这一格里、属于主导区域的像素的平均色（默认；同一区域里相邻格子跟着局部明暗走，
  细节最多又不噪）、**center** 区域中心那一颗原像素（参考实现的做法，对比强，但同区域仍同色）、
  **vote** 区域内出现最多的调色板色、**mean** 区域平均色（最平涂，最初只有这一种，
  被用户评价为"完全没有细节"）。
- 网页上"高级参数"面板把 step/iters/weight/mode 四个旋钮交给用户，并带一块
  **实时预览**（40x40 放大 4 倍、用设备那张 16 色，改完 100 ms 内出图）；
  三档预设只是这几个数的组合，动过滑块之后档位显示"自定义"。预览源优先用刚选的照片，
  没选过就用设备上已有的那张自定义头像（相当于"把这张头像再走一遍像素化"）。
...
- 许可：图标是 **Twemoji 图形**（https://github.com/jdecked/twemoji，tag v16.0.0），
  ...
  作品，采用本仓库许可证。
- 代价：图标、底纹与调色板合计约 119 KB 源码 / 18 KB Flash（图标 15.2 KB，
  即 18 张 864 字节的 I4 图；底纹 2,304 字节）；I4 图标在绘制时按行转换，不整屏缓存。
