...
- 图标在 2026-09-18 重画过其中 4 个：**bird / dog / rabbit / tree**。它们的轮廓在
  40px 下读不出来（bird 是一团黄色、rabbit 像杯子、dog 的耳朵像一对犄角、tree 的红色
  树干长在树中间）。新掩码由 AI 生图工具出的设计稿作参考、**手工重排到 8×8 栅格**
  （模型的"像素"并不落在真实栅格上、描边是发丝细，直接降采样会糊，所以没法自动转）。
  其余 12 个保持不变：它们本身已经清楚，我重画的草稿反而更差。改这 4 个时**没有**改动
  图标顺序——`LOVE_ICON_*` 的下标存在用户配置里，换序等于把用户的图标换掉。
- 生成结果：
