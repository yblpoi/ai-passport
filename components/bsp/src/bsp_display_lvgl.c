...
lv_display_t *bsp_lvgl_init(void) {
    ...
    const lvgl_port_display_cfg_t dc = {
        ...
        // 40 行单缓冲 ≈ 19.2KB。整屏 320 行因此切成 8 带(20 行时是 16 带):每带的
        // 渲染与发送都翻倍,一屏总时间不变,少掉的是肉眼看到的那种"逐行擦除"阶梯。
        // 双缓冲(≈37.5KB)不行 —— 它会把 I2S 等外设的 DMA 描述符挤到 NO_MEM,
        // 所以 double_buffer 仍是 false。
