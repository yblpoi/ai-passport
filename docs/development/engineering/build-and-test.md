...
`packAvatarPalette()`/`paletteOf()`, and parses `admin.js` with all five
...
The page's countdown preview needs the same rules as the device, so it is checked
against the device rather than against hand-written expectations.
`tests/test_preview_math.mjs` runs the preview kernel that ships
(`assets/web/preview_math.js`) and compares it with `tests/vectors/date_vectors.json`;
that vector file is generated by `tools/gen_date_vectors.py`, which compiles and runs
the device's own `love_date.c` and `love_lunar.c`. `--check` verifies the committed
vectors still match the C implementation, so editing a rule on one side without the
other fails the gate.

`tests/test_gen_admin_page.py` keeps the embedded admin page honest: the generated
`main/love_admin_page.h` must match a fresh render of `assets/web/*` (changing the
page without running `tools/gen_admin_page.py` is a common mistake), each gzip
stream must decompress to exactly the bytes the device would serve, and the packed
total must stay inside the documented budget.
