...
- **二次开发必须重新设计 UI**：禁止沿用当前 demo 测试页面或 `ui_pixel` 界面外壳，改名、换颜色不算重新设计；遵循[强制 UI 重新设计规则](../ai-guide.zh_CN.md#二次开发-ui-强制重新设计)。保留既有主题仅适用于维护基线硬件测试 demo 本身，不适用于应用开发。
- **默认右上角电量**：用户界面默认在右上角显示电量信息，除非开发者指定其他位置或明确不需要。读取 `bsp_battery_soc()`（需要时用 `bsp_battery_mv()`），以小型电池图标或百分比展示；读值为 `-1`（不可用）时优雅降级。避免遮挡应用自己的内容；维护基线主题时，还需避开白云装饰（`add_cloud`，约 `x≈188, y≈8`）。
