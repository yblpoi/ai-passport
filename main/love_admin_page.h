...
static const char LOVE_ADMIN_HTML[] =
    ...
    "      <option value=\"1\">单页（每个事件独占一屏）</option>\n"
    "      <option value=\"0\">列表（一屏 4 条，可按分类分组）</option>\n"
    "    </select>\n"
    "    <p class=\"muted\">列表模式下，主屏按上/下键进入事件列表：一屏 4 条、分页显示，选中一条按确定进入它的单页卡，\n"
    "      在卡片里翻到头会退回列表。<b>同一分类的事件会排在一起</b>，分组顺序由下面事件卡的上移/下移决定\n"
    "      （取每个分类里第一条的位置），组内按“距今远近”排，农历超出范围的排最后。没有分类的事件统一排在最后。</p>\n"
    ...
    "      <b>分类</b>留空表示不分类；填过的分类名会在输入时提示，也可以点卡片的“收起”把长列表压短再上下移。</p>\n"
    "  </div>\n"
    "\n"
    "  <div class=\"card\">\n"
    "    <h2>时间</h2>\n"
