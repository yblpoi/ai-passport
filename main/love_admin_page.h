...
static const char LOVE_ADMIN_HTML[] =
    ...
    "    <!-- 只影响\"往空槽传照片\"时的处理：设备端存的还是 40x40、16 色，网页这边用\n"
    "         SLIC 超像素把照片按区域抹平成像素画，档位决定超像素的粗细。 -->\n"
    "    <label for=\"avatarStrength\">照片像素化强度<span class=\"muted\">（上传照片时使用）</span></label>\n"
    "    <select id=\"avatarStrength\">\n"
    "      <option value=\"soft\" selected>弱：保留更多细节</option>\n"
    "      <option value=\"normal\">标准</option>\n"
    "      <option value=\"strong\">强：大块平涂，更像海报</option>\n"
    "      <option value=\"custom\" hidden>自定义（见下面的高级参数）</option>\n"
    "    </select>\n"
    "    <!-- 高级参数：三个旋钮 + 上色方式。照参考实现（xs7/Pixelate 的 dat.GUI）给的，\n"
    "         区别是这里直接给\"设备那 16 色 + 40x40\"下的实时预览，改完一眼能看到。 -->\n"
    "    <details class=\"adv\" id=\"advBox\">\n"
    "      <summary>高级参数<span class=\"muted\">（改完即时预览；三档预设只是它们的组合）</span></summary>\n"
    "      <div class=\"advrow\">\n"
    "        <label for=\"advStep\">超像素大小 <b id=\"advStepVal\">8</b><span class=\"muted\"> px</span></label>\n"
    "        <input type=\"range\" id=\"advStep\" min=\"4\" max=\"32\" step=\"1\" value=\"8\">\n"
    "      </div>\n"
    "      <div class=\"advrow\">\n"
    "        <label for=\"advIters\">迭代次数 <b id=\"advItersVal\">5</b></label>\n"
    "        <input type=\"range\" id=\"advIters\" min=\"1\" max=\"12\" step=\"1\" value=\"5\">\n"
    "      </div>\n"
    "      <div class=\"advrow\">\n"
    "        <label for=\"advWeight\">颜色权重 <b id=\"advWeightVal\">12</b><span class=\"muted\">（越小越按位置分块）</span></label>\n"
    "        <input type=\"range\" id=\"advWeight\" min=\"4\" max=\"48\" step=\"1\" value=\"12\">\n"
    "      </div>\n"
    "      <label for=\"advMode\">上色方式<span class=\"muted\">（一片区域该填什么色）</span></label>\n"
    "      <select id=\"advMode\">\n"
    "        <option value=\"cell\" selected>格子内该区域的平均色：细节最多</option>\n"
    "        <option value=\"center\">区域中心那一颗原像素：对比强，同区域仍同色</option>\n"
    "        <option value=\"vote\">区域内最多的调色板色：最典型的像素画</option>\n"
    "        <option value=\"mean\">区域平均色：最平滑、最平涂</option>\n"
    "      </select>\n"
    "      <div class=\"advpreview\">\n"
    "        <canvas id=\"advPreview\" width=\"160\" height=\"160\"></canvas>\n"
    "        <p class=\"muted\" id=\"advHint\">先在这里选一张照片，或直接调参数看效果（预览对应当前这张图的 40x40 结果）。</p>\n"
    "      </div>\n"
    "      <div class=\"row\" style=\"margin-top:8px\">\n"
    "        <button type=\"button\" class=\"accent\" id=\"advReupload\" hidden>用当前参数重新上传这张</button>\n"
