...
static const char LOVE_ADMIN_HTML[] =
    ...
    "    </div>\n"
    "    <label for=\"bleEnabled\">蓝牙对时</label>\n"
    "    <select id=\"bleEnabled\">\n"
    "      <option value=\"1\">开（可用 nRF Connect 对时）</option>\n"
    "      <option value=\"0\">关（出厂默认，省内存）</option>\n"
    "    </select>\n"
    "    <p class=\"muted\">设备没有时钟。联网后会自动走 SNTP；也可以点上面的按钮用手机时间来对时（不依赖外网）。\n"
    "      蓝牙只用于对时，默认关闭：关着时上面两种对时都不受影响，需要 nRF Connect 那种对时方式时再打开。</p>\n"
    "  </div>\n"
    "\n"
    "  <div class=\"card\">\n"
    "    <h2>网络与热点</h2>\n"
    ...
    "    </div>\n"
    "    <p class=\"muted\">BLE 对时：先在「时间」里把蓝牙对时打开，再用 nRF Connect 等工具连接设备，向服务 A001 的 A002 特征写入 Unix 时间戳（秒，十进制文本或 8 字节小端）即可。</p>\n"
    "  </div>\n"
    "</div>\n"
