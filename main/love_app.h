...
//
// 开机直接进入本应用,没有中间菜单。视图分两类:
//   主屏  在一起 N 天 + 两个人物
//   轮播  上/下 翻页(连按两次"上"一步回主页):先是各张单页事件卡(倒计时),
//         再是列表屏各页;越出两端回主屏,所以 主屏-单页-列表页-主屏 是一个闭合的圈。
//         页码是这一整圈里的第几页(规则与实现见 love_view.h)。
//         熄屏后的第一下只亮屏、不执行动作(判定规则见 love_key.h)。
...
// 现在停在哪一屏,写成给人看的一行(主屏 / 列表页(列表组第 2/2 页,轮播第 2/3 页)/
// 单页卡「咕咕嘎嘎」(轮播第 1/3 页)/ 设置页 …)。
//
// 存在的理由和"它为什么不睡"那条一样:上/下 翻到哪一页是内存里的状态,没有屏幕时
// 只能靠**再截一张图**才知道 —— 而每次打开串口都会复位芯片,截图这条路验不了连续翻页。
// 有这一行就能在旁边敲 key down / status,一步步看着页码走。
// **可以在非 LVGL 任务上调用**(内部自己取锁)。
