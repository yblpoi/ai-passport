...
// 顺序是:先扫描,在已保存的热点里挑信号最强的连;一个都没扫到(包括对方的 SSID 是
// 隐藏的)就逐个试 —— 其余候选里这一轮**扫到过的**优先(信号强的先),都不在扫描结果里
// 才按保存顺序,每个候选 15 秒。整轮都没连上就退避重来(30 秒起,翻倍到 5 分钟),
// 退避期间**不重连**;但每条候选失败(或 15 秒里一个事件都没来)都留一行日志,整轮再一条
// 汇总(汇总按 ROUND_LOG_MIN_INTERVAL_MS 节流,稳态下 5 分钟一条)。
//
...
// 开机自动开、深睡醒来恢复、联网失败兜底这三条自动路径都会让路,直到用户手动开一次
// (或清除凭据)。
