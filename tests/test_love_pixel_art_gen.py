...
class IconInvariantsTest(unittest.TestCase):
    ...
    def test_bg_tile_is_an_a8_mask_at_the_requested_opacity(self):
        """爱心底纹是 A8 遮罩:每像素 1 字节的不透明度,颜色由 style 的 recolor 给。

        格式是实测定下来的,不是随便挑的:同一张图案用 RGB565(不透明)一屏约 104 ms、
        用 I1 索引图约 400 ms(每带 45~56 ms),而 A8 走的是
        lv_draw_sw_img.c 里"把整张图当遮罩、用 recolor 颜色填充"的快路径。
        所以这里钉住的是**设备侧那份 C 数据的格式契约**:
        data 是 48x48 的 alpha 字节、stride = 48(字节/行),没有调色板。
        索引图的 stride 是"字节/行 = 48/8",两者写混就会整屏斜纹。
        """
        text = (ROOT / "assets/images/love_pixel_art.c").read_text(encoding="utf-8")
        self.assertRegex(text, r"static const uint8_t bg_tile_data\[2304\] = \{")
        self.assertRegex(text, r"\.cf = LV_COLOR_FORMAT_A8")
        self.assertRegex(text, r"\.w = 48, \.h = 48, \.stride = 48 \}")
        self.assertRegex(text, r"\.data_size = sizeof\(bg_tile_data\)")

        match = re.search(r"static const uint8_t bg_tile_data\[2304\] = \{(.*?)\n\};",
                          text, re.S)
        self.assertIsNotNone(match)
        data = [int(v, 16) for v in re.findall(r"0x([0-9A-F]{2})", match.group(1))]
        self.assertEqual(len(data), GEN.BG_TILE_PX * GEN.BG_TILE_PX)
        ...
        for y in range(GEN.BG_TILE_PX):
            for x in range(GEN.BG_TILE_PX):
                expected = GEN.BG_HEART_ALPHA if built[y][x][3] else 0
                self.assertEqual(data[y * GEN.BG_TILE_PX + x], expected,
                                 f"({x},{y}) 的遮罩值与 build_bg_tile() 不一致")
